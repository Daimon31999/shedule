...
const timer = (
  ...
  return new Promise((resolve) => {
    function x() {
      ...
      let shedule_arr = shedule.split('\n')

      console.clear()
      
      let str = 
      `
${(1 === para ? `${COLOR.FORE_RED_BOLD}1) ${shedule_arr[0]} < ${COLOR.RESET}`: `1) ${shedule_arr[0]}`)}
${(2 === para ? `${COLOR.FORE_RED_BOLD}2) ${shedule_arr[1]} < ${COLOR.RESET}`: `2) ${shedule_arr[1]}`)}
${(3 === para ? `${COLOR.FORE_RED_BOLD}3) ${shedule_arr[2]} < ${COLOR.RESET}`: `3) ${shedule_arr[2]}`)}
${(4 === para ? `${COLOR.FORE_RED_BOLD}4) ${shedule_arr[3]} < ${COLOR.RESET}`: `4) ${shedule_arr[3]}`)}
${(5 === para ? `${COLOR.FORE_RED_BOLD}5) ${shedule_arr[4]} < ${COLOR.RESET}`: `5) ${shedule_arr[4]}`)}
...
const app = async () => {
  ...
  // Прочитать shedule.txt в shedule
  let fs = require('fs')
  let shedule = fs.readFileSync('shedule.txt', 'utf8')
  let cp_shedule = shedule.split('\n')
  let time_remaining = 0

  // Считать время перемены
  const peremena = (para) => {
    let row1 = cp_shedule[para - 1]
    let row2 = cp_shedule[para]  
    try {  
      // hmax = 9
      let hmax1 = row1.split('-')[1].split(':')[0]
      // mmax = 30
      let mmax1 = row1.split('-')[1].split(':')[1]

      // hmax = 9
      let hmax2 = row2.split('-')[0].split(':')[0]
      // mmax = 40
      let mmax2 = row2.split('-')[0].split(':')[1]
    } catch {
      // Пары закончились
      console.clear()
    ...
    let date1 = new Date((hmax1 * 60 * 60 * 1000) + (mmax1 * 60 * 1000)).getTime()
    let date2 = new Date((hmax2 * 60 * 60 * 1000) + (mmax2 * 60 * 1000)).getTime()
  ...
  for (let i = 0; i < cp_shedule.length; i++) {
    // row = 8:00-9:30
    let row = cp_shedule[i]
    // hmin = 8
    ...
    // перемена
      ...
      await timer(time_remaining, para++, 'осталось', 'перемена', shedule)
      time_remaining = 90
      break
    }
  }
  ...
  while (para < 6) {
    await timer(time_remaining, para, `осталось`, `пара`, shedule)
    await timer(peremena(para), para++, 'осталось', 'перемена', shedule)
    // await timer(0.05, para, `осталось`, `пара`, shedule)
    // await timer(peremena(para), para++, 'осталось', 'перемена', shedule)
  }
  // Пары закончились
  console.clear()
